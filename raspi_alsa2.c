...
//  ToneGenerator
//
//  Created by 長谷部 雅彦 on 2013/04/13.
//  Copyright (c) 2013年 長谷部 雅彦. All rights reserved.
//
//		Original Source is pcm.c of ALSA demo
                      ...
                      snd_pcm_channel_area_t *areas)
{
	...
	while (1) {
		generate_wave(areas, 0, period_size, &phase);
		ptr = samples;
		cptr = period_size;
		while (cptr > 0) {
...
static void async_callback(snd_async_handler_t *ahandler)
{
	...
	while (avail >= period_size) {
		generate_wave(areas, 0, period_size, &data->phase);
...
static void async_direct_callback(snd_async_handler_t *ahandler)
{
	...
	while (1) {
		...
		while (size > 0) {
			...
			generate_wave(my_areas, offset, frames, &data->phase);
